...
class FormatUtilsTest {
    ...
    void testReplacePlaceholdersWithDifferentTypes() {
        // 不同类型的参数
        String result = FormatUtils.replacePlaceholders("字符串: {}, 数字: {}, 布尔: {}, 小数: {}", "测试", 42, true, 3.14);
    ...
    void testReplaceNamedPlaceholders() {
        ...
        // 命名占位符
        String result = FormatUtils.replaceNamedParameters("用户 {name} 年龄 {age} 岁，来自 {city}", params);
        assertEquals("用户 王五 年龄 28 岁，来自 北京", result);

        // 部分匹配
        params.clear();
        params.put("name", "赵六");
        result = FormatUtils.replaceNamedParameters("用户 {name} 年龄 {age} 岁", params);
    ...
    void testReplaceIndexedPlaceholders() {
        // 索引占位符
        String result = FormatUtils.replaceIndexedParameters("参数0: {0}, 参数1: {1}, 参数0再次: {0}", "第一个", "第二个");
        assertEquals("参数0: 第一个, 参数1: 第二个, 参数0再次: 第一个", result);

        // 索引超出范围
        result = FormatUtils.replaceIndexedParameters("参数0: {0}, 参数1: {1}, 参数2: {2}", "第一个", "第二个");
    ...
    void testFormatWithLocale() {
        ...
        String result = FormatUtils.replacePlaceholders("数字: {}, 小数: {}", chineseLocale, timeZone, 1234, 3.14159);
    ...
    void testSpecialCharactersInTemplate() {
        // 模板中包含特殊字符
        String result = FormatUtils.replacePlaceholders("JSON: {\"key\": \"{}\"}", "value");
        assertEquals("JSON: {\"key\": \"value\"}", result);

        // 包含换行符
        result = FormatUtils.replacePlaceholders("第一行: {}\n第二行: {}", "内容1", "内容2");
    ...
    void testComplexObjects() {
        ...
        String result =
                FormatUtils.replacePlaceholders("时间: {}, 金额: {}, 对象: {}", now, amount, new TestObject("测试", 42));
